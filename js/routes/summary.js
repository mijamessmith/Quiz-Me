...
router.get("/postGame", (req, res, next) => {
    let score = req.query.score;
    return db.pool.query(`INSERT INTO user_scores (userId, score, targetUser) VALUES ("${req.session.currentUserId}", "${score}", "${req.session.targetPerson}")`, function (err, result) {
        if (err) {
            throw err;
        } else {
